...
REAL ReturnDepth(REAL x, REAL y) {
  ...
  if(x<=1.675)
    d=0.56;
  else if(x>1.675 && x<=1.675+R*s/sqrt(1+s*s))
    d=0.56-(R-sqrt(R*R-(x-1.675)*(x-1.675)));
  else
    d=0.56-(R-R/sqrt(1+s*s))-(x-(1.675+R*s/sqrt(1+s*s)))*s;
  return d;
}
...
REAL ReturnSalinity(REAL x, REAL y, REAL z) {
  REAL alpha_s=0.99,delta=0.02,a=0.1, h1=0.30;
  REAL rho_diff=0.03,beta=1e-3;
  REAL d,s,R=3;
  REAL L_rho=0.7, eta=a*exp(-x*x/L_rho/L_rho);
