...
void BoundaryScalars(gridT *grid, physT *phys, propT *prop, int myproc, MPI_Comm comm) {
  ...
  z-=grid->dzz[ib][k]/2;
        if(z<-40)
    phys->boundary_s[jptr-grid->edgedist[2]][k]=1;
        else
...
void BoundaryVelocities(gridT *grid, physT *phys, propT *prop, int myproc, MPI_Comm comm) {
  ...
  for(jptr=grid->edgedist[2];jptr<grid->edgedist[3];jptr++) {
    ...
    for(k=grid->etop[j];k<grid->Nke[j];k++) {
      ...
      z-=grid->dz[k]/2;
      phys->boundary_u[jptr-grid->edgedist[2]][k]=ReturnHorizontalVelocity(0,0,1,1,0);
...
void UserDefinedFunction(gridT *grid, physT *phys, propT *prop,int myproc)
{
  ...
  for(i=0;i<grid->Nc;i++)
    for(k=0;k<grid->Nk[i];k++){
      if(phys->s[i][k]<0)
        sum1+=grid->dzz[i][k]*grid->Ac[i];
      if(phys->s[i][k]>0)
        sum2+=grid->dzz[i][k]*grid->Ac[i];
    }
}
