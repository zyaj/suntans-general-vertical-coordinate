  ...
  for(jptr=grid->edgedist[2];jptr<grid->edgedist[3];jptr++) {
    ...
    ib = grid->grad[2*j];
      for(k=grid->etop[j];k<grid->Nke[j];k++) {
        ...
        phys->boundary_u[jptr-grid->edgedist[2]][k]=0.134*sin(1.41e-4*prop->n*prop->dt);
      }
  }
...
void UserDefinedFunction(gridT *grid, physT *phys, propT *prop,int myproc)
{
  ...
  REAL sum=0,s1,s2,z1,z2,s0=4.99541;
  for(i=0;i<grid->Nc;i++){
    sum=0;
    found=0;
    if(prop->vertcoord==2)
      for(k=grid->ctop[i];k<4;k++)
      {
        sum+=grid->dzz[i][k];
      }
    if(prop->vertcoord==5){
      ...
      {
        if(phys->s[i][k]>s0 && found==0){
          found=1;
          s1=phys->s[i][k-1];
          s2=phys->s[i][k];
          z1=vert->zc[i][k-1];
          z2=vert->zc[i][k];
          sum=z2+(z1-z2)/(s1-s2)*(s0-s2);
        }
      }
      if(found==0)
        sum=-grid->dv[i];
