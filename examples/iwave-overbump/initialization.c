...
REAL ReturnSalinity(REAL x, REAL y, REAL z) {
  REAL drho=0.0050939253,h0=224,p=1.15,s1=-0.00018,s2=-3.64e-8,beta=1e-3,s;
  if(z<-2250)
    z=-2250;
...
REAL IsoReturnSalinity(REAL x, REAL y, REAL z, int i,int k) {
  REAL drho=0.0050939253,h0=224,p=1.15,s1=-0.00018,s2=-3.64e-8,beta=1e-3,s,zp;
    if(k==0)
      zp=-25.0;
    if(k==1)
      zp=-75.0;
    ...
    if(k==9)
      zp=-2250.0;
  // here rho0=1024.75 drho=5.22
  s=drho/beta*(1-pow(exp(zp/h0),p)+s1*zp+s2*zp*zp);
  return s;
}
