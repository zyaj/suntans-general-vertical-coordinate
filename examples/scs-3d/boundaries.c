...
void BoundaryVelocities(gridT *grid, physT *phys, propT *prop, int myproc, MPI_Comm comm) {
  int j, jind, jptr, n, k;
  REAL h, u, v, toffSet, secondsPerDay = 86400.0, ramptime=86400;
  ...
  for(jptr=grid->edgedist[2];jptr<grid->edgedist[3];jptr++) {
    ...
    phys->boundary_h[jind]=h*(1-exp(-prop->rtime/ramptime))/100.0;
    for(k=grid->etop[j];k<grid->Nke[j];k++) {
      phys->boundary_u[jind][k]=u*(1-exp(-prop->rtime/ramptime))/100.0;
      phys->boundary_v[jind][k]=v*(1-exp(-prop->rtime/ramptime))/100.0;
