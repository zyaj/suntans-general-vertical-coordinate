...
void MomentumSource(REAL **usource, gridT *grid, physT *phys, propT *prop) {
  ...
  if(prop->sponge_distance) {
    for(j=0;j<grid->Ne;j++){
      //if(grid->mark[j]==2 || grid->mark[j]==3){
	...
	usource[j][k]-=prop->dt*exp(-4.0*rSponge[j]/prop->sponge_distance)/
      ...
      //printf("%e\n",usource[j][100]);
      //if((nc1==125 || nc1==123)&& 
      //if(grid->xe[j]<3.14e4 && grid->xe[j]>3.12e4)
      //	printf("Ne=%d, j=%d mark=%d xe=%e ubar=%e rsponge=%e u=%e dzf=%e\n",grid->Ne,j,grid->mark[j],grid->xe[j],ubar,rSponge[j],phys->u[j][10],grid->dzf[j][0]);
      //fflush(stdout);
      }
  }
}
