...
void InitializeIsopycnalCoordinate(gridT *grid, propT *prop, physT *phys,int myproc)
{
  ...
  REAL ratio=1.0/Nkmax,a0=0.01305*6,eta,L=6;
  for(i=0;i<grid->Nc;i++)
  {
    eta=a0*grid->xv[i]/L*2;
    for(k=0;k<grid->Nk[i]/2;k++)
      grid->dzz[i][k]=(0.203-eta)/grid->Nk[i]*2;
    for(k=grid->Nk[i]/2;k<grid->Nk[i];k++)
      grid->dzz[i][k]=(0.087+eta)/grid->Nk[i]*2;
    //grid->dzz[i][0]=0.203-eta;
    //grid->dzz[i][1]=0.087+eta;
  }
}
