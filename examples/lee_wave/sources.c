...
void MomentumSource(REAL **usource, gridT *grid, physT *phys, propT *prop) {
  ...
  REAL Coriolis_f, ubar, depth_face, z, z_s=-943-0*1571-0*2827 -0*1580, delta_s=0*50+1*314, a0, xe;
  REAL fx, fz, *force = phys->a, a1, a2, L = 291600, delta_x = 5000, x_s = 1e5, FORCE=0;

  // REAL U=0.88, N=0.007, D=6911, h=1, lambda = 6075, K = 2*PI/lambda;
  // // FORCE = 3.141592654*U*N*h*h/L/D/4; // queney's witch forcing
  // // FORCE = K*h*h*U*U*sqrt(N*N/U/U - K*K)/2/D; // gill's sin forcing
...
REAL uvol = 0, cellvol = 0, cumvol = 0, myuvol=0, mycumvol=0, u0=0.2, S;
// for(jptr=grid->edgedist[0];jptr<grid->edgedist[1];jptr++) {
//   j = grid->edgep[jptr];
//   nc1 = grid->grad[2*j];
//   nc1 = grid->grad[2*j+1];
...
if(prop->subgrid){
  for(iptr=grid->celldist[0];iptr<grid->celldist[1];iptr++) {
    ...
    for(k=grid->ctop[i];k<grid->Nk[i];k++){
        cellvol = subgrid->Acceff[i][k]*grid->dzz[i][k];
