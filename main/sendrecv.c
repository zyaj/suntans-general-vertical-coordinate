...
/*
 * Function: ISendRecvEdgeData2D
 * Usage: ISendRecvEdgeData2D(wave->Uwind,grid,myproc,comm);
 * ----------------------------------------------------
 * This function will transfer the 2D edge data back and forth between
 * processors using onblocking sends/recvs.
 *
 */
void ISendRecvEdgeData2D(REAL *edgedata, gridT *grid, int myproc, MPI_Comm comm)
{
  int k, n, neigh, neighproc;
  REAL t0=Timer();
