...
static void OpenDataFiles(int myproc) {
  ...
  if(strlen(ProfileVariables)>0) {
    ...
    if(ContainsCharacter(ProfileVariables,'t')) {
      MPI_GetFile(filename,DATAFILE,"tbFile","OpenDataFiles",myproc);
      if(myproc==0) {
	sprintf(str,"%s.prof",filename);
	TaubProfFID = fopen(str,"w");
      }
    }

    if(ContainsCharacter(ProfileVariables,'d')) {
      MPI_GetFile(filename,DATAFILE,"Sediment1File","OpenDataFiles",myproc);
      if(myproc==0) {
	sprintf(str,"%s.prof",filename);
	SedimentProfFID = fopen(str,"w");
      }
    }
...
static void WriteAllProfileData(gridT *grid, physT *phys, propT *prop, MPI_Comm comm, int numprocs, int myproc) {
 ...
 if(ContainsCharacter(ProfileVariables,'t')) 
    Write2DData(sediments->Seditb,merge_tmp,merge_tmp2,TaubProfFID,comm,numprocs,myproc);

  if(ContainsCharacter(ProfileVariables,'d')) 
    for(nsize=0;nsize<sediments->Nsize;nsize++)
      Write3DData(sediments->SediC[nsize],grid->Nk,NkmaxProfs,merge_tmp,merge_tmp2,SedimentProfFID,comm,numprocs,myproc);
