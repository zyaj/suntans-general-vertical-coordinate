...
void InterpolateSubgridDepth(gridT *grid,physT *phys, int myproc,int numprocs);
...
void AllocateandInitializeSubgrid(gridT *grid, propT *prop, int myproc) {
  ...
  subgrid->Asedi_v = (REAL *)SunMalloc(grid->Nc*sizeof(REAL),"AllocateSubgrid");
  subgrid->Asedi_nv = (REAL *)SunMalloc(grid->Nc*sizeof(REAL),"AllocateSubgrid");
  ...
  subgrid->Vsedi_v = (REAL *)SunMalloc(grid->Nc*sizeof(REAL),"AllocateSubgrid");
  subgrid->Vsedi_nv = (REAL *)SunMalloc(grid->Nc*sizeof(REAL),"AllocateSubgrid");
  ...
  if(subgrid->erosionpara)
  {
    subgrid->tau_c_x= (REAL *)SunMalloc(grid->Nc*sizeof(REAL),"AllocateSubgrid");
    subgrid->tau_c_y= (REAL *)SunMalloc(grid->Nc*sizeof(REAL),"AllocateSubgrid");

    subgrid->stmp1_x= (REAL *)SunMalloc(grid->Nc*sizeof(REAL),"AllocateSubgrid");
    subgrid->stmp2_x= (REAL *)SunMalloc(grid->Nc*sizeof(REAL),"AllocateSubgrid");
    subgrid->stmp1_y= (REAL *)SunMalloc(grid->Nc*sizeof(REAL),"AllocateSubgrid");
    subgrid->stmp2_y= (REAL *)SunMalloc(grid->Nc*sizeof(REAL),"AllocateSubgrid");
    
    subgrid->Cdmean = (REAL *)SunMalloc(grid->Nc*sizeof(REAL),"AllocateSubgrid");
    subgrid->Cdmean_v = (REAL *)SunMalloc(grid->Nc*sizeof(REAL),"AllocateSubgrid");
    subgrid->Cdmean_nv = (REAL *)SunMalloc(grid->Nc*sizeof(REAL),"AllocateSubgrid");
    //subgrid->epslon= (REAL *)SunMalloc(grid->Nc*sizeof(REAL),"AllocateSubgrid");
    //tmp=MPI_GetValue(DATAFILE,"subgridepslon","AllocateSubgrid",myproc); 
  }
  ...
  for(i=0;i<grid->Nc;i++)
  {
    ...
    if(subgrid->erosionpara)
      if(tmp>0)
        subgrid->epslon[i]=tmp;
      else
        subgrid->epslon[i]=ReturnSubgridErosionParameterizationEpslon(grid->xv[i],grid->yv[i]);
    */   
    subgrid->delta[i]=tmp;
    if(subgrid->erosionpara)
    {
      ...
      subgrid->stmp2_x[i]=0.0;
      subgrid->stmp2_y[i]=0.0;
    }
    ...
    subgrid->Asedi[i]=0.0;
    subgrid->Vsedi[i]=0.0;
    subgrid->Asedi_v[i]=0.0;
    subgrid->Vsedi_v[i]=0.0;
    subgrid->Asedi_nv[i]=0.0;
    subgrid->Vsedi_nv[i]=0.0;
    subgrid->Veff[i]=0.0;
...
void SubgridBasic(gridT *grid, physT *phys, propT *prop, int myproc, int numprocs, MPI_Comm comm)
{
  ...
  InterpolateSubgridDepth(grid, phys,myproc,numprocs);
...
void UpdateSubgridAceff(gridT *grid, physT *phys, propT *prop, int myproc)
{
  ...
  for(nc=0;nc<grid->Nc;nc++)
  {
    ...
    if(prop->culvertmodel)
      if(h>Culverttop[nc])
        subgrid->Aceff[nc]=UpdateAceff(nc,Culverttop[nc]);
    if(prop->wetdry)
...
void SubgridCulverttopArea(gridT *grid, propT *prop, int myproc)
{
  int nc;
  for(nc=0;nc<grid->Nc;nc++)
    if(Culverttop[nc]!=INFTY)
      Culverttoparea[nc]=UpdateAceff(nc,Culverttop[nc]);
    else  
      Culverttoparea[nc]=0;
}
    ...
    for(i=0;i<grid->Nc;i++)
    {
      ...
      {   
        ...
        {  
          ...
          //subgrid->Acveff[i][k]=UpdateAceff(i,ztop);
          //if(subgrid->Acceff[i][k]==0)
            //printf("nc %d k %d acc=0\n", i,k);
          if(k!=grid->ctop[i])
          {
            zbot=ztop;
            ztop=zbot+grid->dzz[i][k-1];
...
void UpdateSubgridFluxHeight(gridT *grid, physT *phys, propT *prop, int myproc)
{
  ...
  for(ne=0;ne<grid->Ne;ne++)
  {
   ...
   // if(prop->culvertmodel==1)
     // if(Culverttop[nc1]!=INFTY && Culverttop[nc2]!=INFTY)
    ...
    //if(ne==7)
      //printf("dsadasda n %d dzf %e nc %d nc_b %d Vnc %e Vnc_b %e h %e dv1 %e dv2 %e u %e s1 %e s2 %e active1 %d active2 %d\n",prop->n, grid->dzf[ne][0],nc,nc_b,subgrid->Veff[nc],\
        //subgrid->Veff[nc_b],h,grid->dv[nc]+phys->h[nc],grid->dv[nc_b]+phys->h[nc_b],u*grid->n1[ne],phys->s[nc][0],phys->s[nc_b][0],phys->active[nc],phys->active[nc_b]);
...
void OutputSubgrid(gridT *grid, physT *phys, propT *prop,int myproc, int numprocs, MPI_Comm comm)
{
  ...
  for(i=0;i<grid->Nc;i++) 
  {
    ...
    for(j=0;j<(((subgrid->segN+1)*(subgrid->segN+2)/2)*(grid->nfaces[i]-2));j++){
      fprintf(ofile,"%d %e %e %e %e %e %e\n",i,subgrid->xp[base+j],subgrid->yp[base+j],subgrid->dp[base+j],grid->xv[i],grid->yv[i],grid->dv[i]);
    }
    //fprintf(ofile,"\n");
  }
  fclose(ofile);
...
/*
 * Function: CalculateActiveErosion(oldest version, no parameterization)
...
{
  REAL h,hcrit,heff,Hcrit,d1,d2,d3,utmp,ratio1,ratio2,ratio, ac,em,alpha,erosionmax,nettmp,epslon,beta;
  int i,j,k; 
  ...
  // beta parameterization for 1D model
  beta=3;
  ...
  for(i=0;i<sediments->Nsize;i++)
  {
    ...
    for(j=0;j<grid->Nc;j++){
      for(k=0;k<sediments->Nlayer;k++)
      {
        ...
        hcrit=sediments->Taue[i][k]/sediments->Seditb[j]*subgrid->Heff[j]/beta-grid->dv[j];
        if(phys->h[j]<=(hcrit+DRYCELLHEIGHT))
          sediments->Erosion[i][j][k]=0;
        else
        {
          ac=UpdateAceff(j,hcrit);
          ratio1=beta*(UpdateVeff(j,hcrit)/ac+phys->h[j]-hcrit)/subgrid->Heff[j];
          ratio2=ac/subgrid->Aceff[j];

          if(sediments->Softhard[k]==0){
          // soft erosion
            // the subgrid approximation may be not correct
            sediments->Erosion[i][j][k]=sediments->E0[i][k]*ratio2*exp(alpha*(ratio1*sediments->Seditb[j]-sediments->Taue[i][k]));
          } else {
            // hard erosion
            ratio=Max(ratio1*sediments->Seditb[j]/sediments->Taue[i][k]-1,0);
            sediments->Erosion[i][j][k]=sediments->E0[i][k]*ratio2*pow(ratio,em);
          }
        }
        if(sediments->Erosion[i][j][k]>erosionmax)
          sediments->Erosion[i][j][k]=erosionmax; 
       // if(sediments->Seditb[j]>sediments->Taue[i][k])
         // printf("sediments- %e r1 %e r2 %e heff %e hcrit %e d %e h %e\n ",sediments->Erosion[i][j][k],ratio1,ratio2,subgrid->Heff[j],hcrit,grid->dv[j],phys->h[j]);
...
/*
 * Function: CalculateSubgridActiveErosion(old version with epslon parameterization)
 * Usage: Calculate the active erosion for 1D model
 * assume a balance between freesurface gradient and bottom drag
...
/*void CalculateSubgridActiveErosion(gridT *grid, physT *phys, propT *prop, int myproc)
{
  ...
  REAL taubtmp2, taubtmp1, epslon, taub, taub0, max, mean;
  ...
  for(j=0;j<grid->Nc;j++)
  {
    epslon=subgrid->epslon[j];
    if(prop->n*prop->dt/3600/24>6.5)
    {
      epslon=1.0;
    }
    //printf("%e %e %e\n",grid->xv[j],grid->yv[j],epslon );
    ...
    taub0=(1+phys->rho[j][grid->Nk[j]-1])*RHO0*subgrid->Cdmean[j]*pow(utmp,2);
    max=0.0;
    mean=0.0;
    n=0;
    for(k=0;k<ntotal;k++)
    {
      taub=taub0*subgrid->Cdratio[base+k]*epslon;
      if(prop->wavemodel)
      {
        if(wprop->FetchModel)
        {
    ...
    for(j=0;j<grid->Nc;j++)
    {
      ...
      for(k=0;k<sediments->Nlayer;k++)
      {
        ...
        if(sediments->Seditb[j]>0.0)
          for(l=0;l<ntotal;l++)
          {
            if(subgrid->taubsub[base+l]>sediments->Taue[i][k])
            {
              ...
              if(sediments->Softhard[k]==0){
                // soft erosion
                sediments->Erosion[i][j][k]+=sediments->E0[i][k]*exp(alpha*(subgrid->taubsub[base+l]-sediments->Taue[i][k]));
              } else {
                // hard erosion
                ratio=Max(subgrid->taubsub[base+l]/sediments->Taue[i][k]-1,0);
        ...
        if(n==0)
          ...
          sediments->Erosion[i][j][k]=sediments->Erosion[i][j][k]/ntotal*grid->Ac[j]/subgrid->Aceff[j];
...
{
  REAL utmp,ratio, em, alpha, erosionmax, nettmp; 
  REAL taubtmp2, taubtmp1, epslon, taub, taub0, max, mean, Hmean,taub1,taub0_v,taub0_nv;
  int i,j,k,l,base,ntotal,n; 
  // coefficient for hard erosion
  em=1.0;
  // coefficient for soft erosion
  ...
  for(j=0;j<grid->Nc;j++)
  {
    ...
    base=j*(grid->maxfaces-2)*subgrid->segN*subgrid->segN;
    ntotal=(grid->nfaces[j]-2)*subgrid->segN*subgrid->segN;
    if(subgrid->erosionpara==1){
      taub0=(1+phys->rho[j][grid->Nk[j]-1])*RHO0*sqrt(pow(subgrid->tau_c_x[j], 2)+pow(subgrid->tau_c_y[j], 2));
    }
    else{
      taub0=(1+phys->rho[j][grid->Nk[j]-1])*RHO0*subgrid->Cdmean[j]*utmp*utmp;
      taub0_v=(1+phys->rho[j][grid->Nk[j]-1])*RHO0*subgrid->Cdmean_v[j]*utmp*utmp;
      taub0_nv=(1+phys->rho[j][grid->Nk[j]-1])*RHO0*subgrid->Cdmean_nv[j]*utmp*utmp;
    }
    ...
    for(k=0;k<ntotal;k++)
    {
      ...
      if(prop->marshmodel)
        if(subgrid->hmarshc[base+k]>0){
          taub=taub0_v*subgrid->Cdratio[base+k];
        }else{
          taub=taub0_nv*subgrid->Cdratio[base+k];          
        }
      if(prop->wavemodel)
      {
        if(wprop->FetchModel)
        {
          if(taub==0){
            ...
            taubtmp1=sqrt(pow((subgrid->tau_c_x[j]*subgrid->Cdratio[base+k]+0.5*(1+phys->rho[j][grid->Nk[j]-1])*
              RHO0*wave->Fw[j]*pow((wave->Waveexcur[j]*2*PI/wave->Twsig[j]),2)*cos(wave->Winddir[j])),2)+
              pow((subgrid->tau_c_y[j]*subgrid->Cdratio[base+k]+0.5*(1+phys->rho[j][grid->Nk[j]-1])*RHO0*wave->Fw[j]*
              pow((wave->Waveexcur[j]*2*PI/wave->Twsig[j]),2)*sin(wave->Winddir[j])),2));
            taubtmp2=sqrt(pow((subgrid->tau_c_x[j]*subgrid->Cdratio[base+k]+0.5*(1+phys->rho[j][grid->Nk[j]-1])*
              RHO0*wave->Fw[j]*pow((wave->Waveexcur[j]*2*PI/wave->Twsig[j]),2)*(-cos(wave->Winddir[j]))),2)+
              pow((subgrid->tau_c_y[j]*subgrid->Cdratio[base+k]+0.5*(1+phys->rho[j][grid->Nk[j]-1])*RHO0*wave->Fw[j]*
              pow((wave->Waveexcur[j]*2*PI/wave->Twsig[j]),2)*(-sin(wave->Winddir[j]))),2));
            taub=Max(taubtmp1,taubtmp2);
    ...
    if(n==0)
    {
      ...
      sediments->Seditbmax[j]=max;
      sediments->Seditb[j]=taub0;//mean/n;
    }
    /*if(sediments->Seditb[j]>0.1){
      //printf("%d %e\n",j,sediments->Seditb[j]);
      //printf("xv %e yv %e utmp %e cd %e cdmean %e dzz %e dv %e h %e H %e taub %e taubj %e para %d\n", grid->xv[j],grid->yv[j],utmp,
      //  pow((log(grid->dzz[j][grid->Nk[j]-1]/prop->z0B)+prop->z0B/grid->dzz[j][grid->Nk[j]-1]-1)/0.41,-2),subgrid->Cdmean[j],grid->dzz[j][grid->Nk[j]-1],
      //  grid->dv[j],phys->h[j],grid->dv[j]+phys->h[j],taub, sediments->Seditb[j],subgrid->erosionpara);
      //printf("tau_c_x %e tau_c_y %e\n",subgrid->tau_c_x[j],subgrid->tau_c_y[j]);
    }*/
...
void CalculateSubgridDragCoef(gridT *grid, physT *phys, propT *prop)
{
   ...
   for(ne=0;ne<grid->Ne;ne++)
   {
     ...
     if(subgrid->dzboteff[ne]>=BUFFERHEIGHT && grid->etop[ne]==grid->Nke[ne]-1 && grid->mark[ne]!=2) //ignore inflow boundary
     {
       for(i=0;i<subgrid->segN;i++)
       {
         ...
         if(h<(-d2+BUFFERHEIGHT))
         {
           ...
           if(prop->marshmodel)
           {
             ...
             if(p>1)
               p=1;
             p1+=Min(subgrid->hmarshe[base+i],Htmp);
             p2+=Htmp;
       ...
       //if(ne==0)
        //printf("cd %e \n", Cd);
       //if(prop->n%3600==0 && grid->xe[ne]>480 && grid->xe[ne]<500 && grid->ye[ne]>-1 && grid->ye[ne]<1)
        //printf("n %d ne %d cd1 %e cd %e p%e cdb %e cdv %e u %e xe %e ye %e\n",prop->n,ne, Cd1,Cd,p,Cdbtmp,CdVtmp,phys->u[ne][0],grid->xe[ne],grid->ye[ne]);      
      //if(n==0)
        // printf("n %d ne %d cd %e There is something wrong of flux height to calculate the mean average drag coefficient.\n",prop->n,ne,Cd);
       /*if(prop->marshmodel)
       {
         p=marsh->hmarshleft[ne]/(0.5*(grid->dzz[nc1][marsh->marshtop[ne]]+grid->dzz[nc2][marsh->marshtop[ne]]));
         CdVtmp=marsh->CdV[ne]*marsh->Alphav*marsh->Rv*marsh->Na*pow(p,2)*H_mean\
           /(1-PI*pow(marsh->Rv,2)*marsh->Na*p)/pow((1-2*marsh->Rv*sqrt(marsh->Na)*p),2);
       }
       // delete the mean marsh effect which will be calculated in marsh model part in phys.c
       Cd=Cd-CdVtmp;*/
...
void CalculateSubgridCdmean(gridT *grid, physT *phys, propT *prop)
{
  ...
  REAL h,hleft,Htmp, Hmean, p, Cdb, Cdv,Cdsum,tmp,A,R,d1,d2,d3,d,Cdmax,Cdmin, Vc,Ac,ac1,r,Vc_v,Vc_nv,Ac_v,Ac_nv,tmp_v,tmp_nv,alpha;//maxratio;
  for(i=0;i<grid->Nc;i++)
  {
    h=phys->h[i];
    Cdmax=0;
    ...
    tmp_nv=0;
    subgrid->Cdmean[i]=0;
    subgrid->Cdmean_v[i]=0;
    subgrid->Cdmean_nv[i]=0;
    ...
    { 
      ...
      if(prop->marshmodel){
        if(subgrid->hmarshc[base+j]>0){
          Vc_v+=subgrid->A_sub[base+j]*subgrid->H_sub[base+j];
          Ac_v+=subgrid->A_sub[base+j];
          if(Cdsum>0)
            tmp_v+=sqrt(subgrid->H_sub[base+j]/Cdsum)*subgrid->H_sub[base+j]*subgrid->A_sub[base+j];
        } else {
          Vc_nv+=subgrid->A_sub[base+j]*subgrid->H_sub[base+j];
          Ac_nv+=subgrid->A_sub[base+j];  
          if(Cdsum>0)
            tmp_nv+=sqrt(subgrid->H_sub[base+j]/Cdsum)*subgrid->H_sub[base+j]*subgrid->A_sub[base+j];                  
        }
      }else{
          Vc_nv+=subgrid->A_sub[base+j]*subgrid->H_sub[base+j];
          Ac_nv+=subgrid->A_sub[base+j];   
    ...
    subgrid->Asedi[i]=Ac;
    subgrid->Vsedi[i]=Vc;
    subgrid->Asedi_v[i]=Ac_v;
    subgrid->Vsedi_v[i]=Vc_v;
    subgrid->Asedi_nv[i]=Ac_nv;
    subgrid->Vsedi_nv[i]=Vc_nv;
    // here assume non-vegetated part has larger depth

    if(tmp>0)
      subgrid->Cdmean[i]=Vc*Vc*Vc/Ac/tmp/tmp;
    if(tmp_v>0)
      subgrid->Cdmean_v[i]=Vc_v*Vc_v*Vc_v/Ac_v/tmp_v/tmp_v;
    if(tmp_nv>0)
      subgrid->Cdmean_nv[i]=Vc_nv*Vc_nv*Vc_nv/Ac_nv/tmp_nv/tmp_nv;

    if(Ac_v>0)
      alpha=sqrt(Vc_v/Vc_nv*Ac_nv/Ac_v*subgrid->Cdmean_nv[i]/subgrid->Cdmean_v[i]);
    else
      alpha=0;

   if(prop->n==55*360 && grid->xv[i]==475)
      printf("Ac %e Vc %e Ac_v %e Vc_v %e Ac_nv %e Vc_nv %e Cdv %e Cdnv %e Cdmean %e alpha %e tmp %e\n",Ac,Vc,Ac_v,Vc_v,Ac_nv,Vc_nv,subgrid->Cdmean_v[i],subgrid->Cdmean_nv[i],subgrid->Cdmean[i],alpha);
    ...
    { 
      if(subgrid->H_sub[base+j]>BUFFERHEIGHT){
       if(prop->marshmodel){
         if(subgrid->hmarshc[base+j]>0 && Vc_v>0)
           subgrid->Cdratio[base+j]*=subgrid->H_sub[base+j]/Vc_v*Ac_v*(Vc_v+Vc_nv)*(Vc_v+Vc_nv)/(alpha*Vc_v+Vc_nv)/(alpha*Vc_v+Vc_nv)*alpha*alpha;
         if(subgrid->hmarshc[base+j]==0 && Vc_nv>0)
           subgrid->Cdratio[base+j]*=subgrid->H_sub[base+j]/Vc_nv*Ac_nv*(Vc_v+Vc_nv)*(Vc_v+Vc_nv)/(alpha*Vc_v+Vc_nv)/(alpha*Vc_v+Vc_nv);
       } else {
         subgrid->Cdratio[base+j]*=subgrid->H_sub[base+j]/Vc_nv*Ac_nv;
       }
      }
    }
...
void FreeSubgrid(gridT *grid, int myproc) {
  ...
  free(subgrid->Aceff);
  free(subgrid->Cdmean);
  free(subgrid->Cdmean_v);
  free(subgrid->Cdmean_nv);
  ...
  free(subgrid->Vsedi);
  free(subgrid->Asedi);
  free(subgrid->Vsedi_v);
  free(subgrid->Asedi_v);
  free(subgrid->Vsedi_nv);
  free(subgrid->Asedi_nv);
...
void SubgridCellAverageTau(gridT *grid,physT *phys,propT *prop, metT *met, int myproc, int numprocs)
{
  ...
  for(iptr=grid->celldist[0];iptr<grid->celldist[1];iptr++) 
  {
    ...
    //if(i==6) 
      //printf("33333tau_c_x %e UC %e\n", subgrid->tau_c_x[6],phys->uc[6][0]); 
