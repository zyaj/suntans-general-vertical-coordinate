...
static void ComputeUDrag(physT *phys, propT *prop, gridT *grid, int myproc, MPI_Comm comm);
...
void AllocatePhysicalVariables(gridT *grid, physT **phys, propT *prop)
{
  ...
  *phys = (physT *)SunMalloc(sizeof(physT),"AllocatePhysicalVariables");
  ...
  (*phys)->u_tilt = (REAL **)SunMalloc(Ne*sizeof(REAL *),"AllocatePhysicalVariables");
  (*phys)->uc_tilt = (REAL **)SunMalloc(Nc*sizeof(REAL *),"AllocatePhysicalVariables");
  (*phys)->vc_tilt = (REAL **)SunMalloc(Nc*sizeof(REAL *),"AllocatePhysicalVariables");
  (*phys)->ut_drag = (REAL *)SunMalloc(Ne*sizeof(REAL),"AllocatePhysicalVariables");
  (*phys)->ub_drag = (REAL *)SunMalloc(Ne*sizeof(REAL),"AllocatePhysicalVariables");
  ...
  for(j=0;j<Ne;j++) {
    ...
    (*phys)->u_tilt[j] = (REAL *)SunMalloc(grid->Nkc[j]*sizeof(REAL),"AllocatePhysicalVariables");
  ...
  (*phys)->htmp4 = (REAL *)SunMalloc(Nc*sizeof(REAL),"AllocatePhysicalVariables");
  (*phys)->htmp5 = (REAL *)SunMalloc(Nc*sizeof(REAL),"AllocatePhysicalVariables");
  ...
  for(i=0;i<Nc;i++) {
    ...
    (*phys)->uc_tilt[i] = (REAL *)SunMalloc(grid->Nk[i]*sizeof(REAL),"AllocatePhysicalVariables");
    (*phys)->vc_tilt[i] = (REAL *)SunMalloc(grid->Nk[i]*sizeof(REAL),"AllocatePhysicalVariables");
...
void InitializePhysicalVariables(gridT *grid, physT *phys, propT *prop, int myproc, MPI_Comm comm)
{
  ...
  */
  ...
  //for(i=0;i<grid->Nc;i++)
  //  for(k=0;k<grid->Nk[i];k++)
  //    printf("i %d k %d dzz %e ctop %d \n",i,k,grid->dzz[i][k],grid->ctop[i]);
  ...
  // initialize u_drag part
  for(j=0;j<grid->Ne;j++)
  {
    phys->ut_drag[j]=0;
    phys->ub_drag[j]=0;
    for(k=0;k<grid->Nke[k];k++)
      phys->u_tilt[j][k]=0;
  }
...
void SetDragCoefficients(gridT *grid, physT *phys, propT *prop) {
  ...
  for(j=0;j<grid->Ne;j++){
    //if(grid->mark[j]==0)
      //printf("j %d cdb %e dzf %e\n",j,phys->CdB[j],grid->dzf[j][0]);
    if(!prop->subgrid){
    ...
    if(grid->xe[j]==205000 && prop->n%360==0){
      nc1=grid->grad[2*j];
      nc2=grid->grad[2*j+1];
      if(grid->xv[nc1]>grid->xv[nc2])
      {
        nc2=nc1;
        nc1=grid->grad[2*j+1];
      }  
      ...
      {
        u_sum+=phys->u[j][k]*grid->dzf[j][k]*grid->df[j];
        A_sum+=grid->dzf[j][k]*grid->df[j];
      }
      u_sum=u_sum/A_sum;
      printf("n %d ne %d cdb %e u %e tb %e u_sum %e A_sum %e dzb %e Nke-etop %d dhdx %e\n",prop->n,j, phys->CdB[j],phys->u[j][grid->Nke[j]-1],\
        phys->CdB[j]*phys->u[j][grid->Nke[j]-1]*phys->u[j][grid->Nke[j]-1],u_sum,A_sum,subgrid->dzboteff[j],grid->Nke[j]-grid->etop[j],\
        (phys->h[nc2]-phys->h[nc1])/50);
...
static void InterpDrag(gridT *grid, physT *phys, propT *prop,int myproc)
{
   ...
   // for z0B
   if(prop->Intz0B==1){
     Nb = MPI_GetSize(INPUTZ0BFILE,"InterpDrag",myproc);
     ...
     fid = MPI_FOpen(INPUTZ0BFILE,"r","InterpDrag",myproc);
     for(n=0;n<Nb;n++) {
    ...
    /* for(n=0;n<grid->Ne;n++){
     ...
     sprintf(str,"%s-edge",INPUTZ0BFILE);
   ...
   if(prop->Intz0T==1){
     ...
     Nt = MPI_GetSize(INPUTZ0TFILE,"InterpDrag",myproc); // change to z0TFILE
     ...
     fid = MPI_FOpen(INPUTZ0TFILE,"r","InterpDrag",myproc);
     for(n=0;n<Nt;n++) {
     ...
     sprintf(str,"%s-edge",INPUTZ0TFILE);
     fid = MPI_FOpen(str,"r","InterpDrag",myproc);
     for(n=0;n<grid->Ne;n++) {
       getfield(fid,str);
       getfield(fid,str);
  ...
  if(grid->Nkmax>1) {
    for(i=0;i<Nc;i++) {
      ...
      for(k=0;k<grid->Nk[i];k++) {
        ...
        if(phys->h[i]>=z) 
          if(!flag) {
            //printf("find ctop correctly i %d k %d\n",i,k);
            if(k==grid->Nk[i]-1) {
...
void Solve(gridT *grid, physT *phys, propT *prop, int myproc, int numprocs, MPI_Comm comm)
{
  ...
  REAL t0,sum1,v_average,sum2,sum3,sum4,sum5,sum6;
  metinT *metin;
  metT *met;
  averageT *average;
  ...
  for(n=prop->nstart+1;n<=prop->nsteps+prop->nstart;n++) {
    ...
    if(prop->nsteps>0) {
      ...
      // compute u_drag for drag forcing
      ComputeUDrag(phys, prop, grid, myproc, comm);
      ...
      // output cross-sectionally average values
      if(prop->n%360==0 || prop->n==1)
      {
        //printf(" %d %d\n",grid->grad[2*142+1],grid->grad[2*142] );
        sum1=0;
        sum2=0;
        sum3=0;
        sum4=0;
        ...
        for(ne=0;ne<grid->Ne;ne++)
        {
          if(grid->xe[ne]==1500)
          {
            for(kk=grid->etop[ne];kk<grid->Nke[ne];kk++)
            {
              sum3+=grid->dzf[ne][kk]*grid->df[ne]*phys->u[ne][kk]*grid->n1[ne];
              sum4+=grid->dzf[ne][kk]*grid->df[ne];
              sum5+=grid->dzf[ne][kk];
            }  
            if(prop->n>53*360 && prop->n<71*360 && ne==142)
            {  
              if(prop->subgrid)
              printf("n %d ne %d dzboteff %e cdb %e tb %e\n",prop->n, ne, subgrid->dzboteff[ne],phys->CdB[ne], phys->CdB[ne]*phys->u[ne][grid->Nke[ne]-1]*phys->u[ne][grid->Nke[ne]-1]);
              else
               printf("n %d ne %d dzboteff %e cdb %e tb %e\n",prop->n, ne, grid->dzf[ne][grid->Nke[ne]-1],phys->CdB[ne], phys->CdB[ne]*phys->u[ne][grid->Nke[ne]-1]*phys->u[ne][grid->Nke[ne]-1]);
               

              printf("dhdx %e ubot %e dzf %e\n",phys->h[grid->grad[2*ne+1]]-phys->h[grid->grad[2*ne]],phys->u[ne][grid->Nke[ne]-1],sum5);
            }
          }  
          if(grid->xe[ne]==2500)
          {
            for(kk=grid->etop[ne];kk<grid->Nke[ne];kk++)
            {
              sum5+=grid->dzf[ne][kk]*grid->df[ne]*phys->u[ne][kk]*grid->n1[ne];
              sum6+=grid->dzf[ne][kk]*grid->df[ne];
            }  
          }          
          if(grid->xe[ne]==500)
          {
            for(kk=grid->etop[ne];kk<grid->Nke[ne];kk++)
            {
              sum1+=grid->dzf[ne][kk]*grid->df[ne]*phys->u[ne][kk]*grid->n1[ne];
              sum2+=grid->dzf[ne][kk]*grid->df[ne];
            }  
          } 
        }
        //printf("n %d x 500 Q %e A %e u %e proc %d\n",prop->n,\
          //sum1,sum2,sum1/sum2,myproc);
        printf("n %d x 1500 Q %e A %e u %e proc %d\n",prop->n,\
          sum3,sum4,sum3/sum4,myproc);
        //printf("n %d x 2500 Q %e A %e u %e proc %d\n",prop->n,\
          //sum5,sum6,sum5/sum6,myproc);
      }
     ...
     // Update the age (passive) tracers
      ...
      if(prop->computeSediments){
        t0=Timer(); 
        // if subgrid erosionpara is on 
        // must be one layer hydrostatic model
        ...
        if(prop->subgrid && grid->Nkmax==1)
          if(subgrid->erosionpara)
          {
            ...
            ISendRecvCellData3D(phys->uc,grid,myproc,comm);
            ISendRecvCellData3D(phys->vc,grid,myproc,comm);
            // add wind stress and phys->uc_new/dt to calculate cell average new
            SubgridCellAverageTau(grid,phys,prop,met,myproc,numprocs);
          }
      ...
      //if(prop->n==1)
        //for(j=0;j<grid->Ne;j++)
          //printf("ne %d cd %e/n", j,phys->CdB[j]);
...
//
  ...
  // to calculate volume averaged tau_c 
  // used in subgrid sediment transport model
  // only works when subgrid and erosionpara is on
  if(prop->subgrid && grid->Nkmax==1)
    if(subgrid->erosionpara){
      for(iptr=grid->celldist[0];iptr<grid->celldist[1];iptr++) 
      {
        i=grid->cellp[iptr];
        subgrid->tau_c_x[i]=0.0;
        subgrid->tau_c_y[i]=0.0;        
        subgrid->tau_c_x[i]=phys->uc[i][0]/prop->dt+fab2*subgrid->stmp1_x[i]+fab3*subgrid->stmp2_x[i];
        subgrid->tau_c_y[i]=phys->vc[i][0]/prop->dt+fab2*subgrid->stmp1_y[i]+fab3*subgrid->stmp2_y[i];        
        subgrid->stmp2_x[i]=subgrid->stmp1_x[i];
        subgrid->stmp2_y[i]=subgrid->stmp1_y[i];
        subgrid->stmp1_x[i]=0.0;
        subgrid->stmp1_y[i]=0.0;   
      }
    }
  ...
  // for subgrid sediment transport 
  // add coriolis forcing for cell centered momentum balance
  if(prop->subgrid && grid->Nkmax==1)
    if(subgrid->erosionpara)
      for(iptr=grid->celldist[0];iptr<grid->celldist[1];iptr++) 
      {
        i=grid->cellp[iptr];
        subgrid->stmp1_x[i]+=phys->vc[i][0]*prop->Coriolis_f;
        subgrid->stmp1_y[i]-=phys->uc[i][0]*prop->Coriolis_f;   
      }  
  ...
  // for subgrid sediment transport 
  // add baroclinic forcing for cell centered momentum balance  
  if(prop->subgrid && grid->Nkmax==1)
    if(subgrid->erosionpara)
      for(iptr=grid->celldist[0];iptr<grid->celldist[1];iptr++) 
      {
        ...
        tmp_y=0.0;
        tmp=0.0;
        // calculate cell volume baroclinic pressure
        for(nf=0;nf<grid->nfaces[i];nf++)
        {
          ...
          if(nc2==-1)
            nc2=nc1;
          tmp=0.25*prop->grav*(grid->dzz[nc1][0]+grid->dzz[nc2][0])*
            (phys->rho[nc1][0]-phys->rho[nc2][0])/grid->dg[ne]*
            grid->def[i*grid->maxfaces+nf]*grid->dzf[ne][0]*grid->df[ne];
          tmp_x+=tmp*grid->n1[ne];
          tmp_y+=tmp*grid->n2[ne];
        }
        subgrid->stmp1_x[i]-=tmp_x/subgrid->Veff[i];
        subgrid->stmp1_y[i]-=tmp_y/subgrid->Veff[i];   
      }  
  ...
  // subgrid sediment transport parameterization
  // add horizontal advection, horizontal diffusion x,y direction
  // still need uc_new/dt, pressure gradient and wind stress 
  if(prop->subgrid && grid->Nkmax==1)
    if(subgrid->erosionpara){
      for(iptr=grid->celldist[0];iptr<grid->celldist[1];iptr++) {
        i=grid->cellp[iptr];
        subgrid->stmp1_y[i]-=phys->stmp2[i][0];
        subgrid->stmp1_x[i]-=phys->stmp[i][0];
        subgrid->tau_c_x[i]+=fab1*subgrid->stmp1_x[i];
        subgrid->tau_c_y[i]+=fab1*subgrid->stmp1_y[i]; 
      } 
    }
 
  //printf("1123131 nc 0 tau_x %e nc 1 tau_x %e nc 2 %e %e %e\n", subgrid->tau_c_x[0],subgrid->tau_c_x[1],subgrid->tau_c_x[2],subgrid->stmp1_x[0],subgrid->stmp1_x[1]);
  ...
  if(prop->nonlinear) {
      ...
      for(nf=0;nf<grid->nfaces[i];nf++) {
        ...
        for(k=grid->ctop[i];k<grid->Nk[i];k++)
          if(!prop->subgrid || prop->wetdry)
            ...
            phys->stmp[i][k]+=phys->ut[ne][k]*phys->utmp2[ne][k]*grid->df[ne]*grid->normal[i*grid->maxfaces+nf]/(a[k]*subgrid->Acceffold[i][k]); // consistent with dzz not dzzold
	...
	if(prop->conserveMomentum)
	  for(k=grid->etop[ne];k<grid->ctop[i];k++)
	      ...
	      phys->stmp[i][grid->ctop[i]]+=phys->ut[ne][k]*phys->utmp2[ne][k]*grid->df[ne]*grid->normal[i*grid->maxfaces+nf]/(a[k]*subgrid->Acceffold[i][k]);
      }
      ...
      if(prop->nonlinear==1 || prop->nonlinear==2 ||prop->nonlinear==5) {
        for(k=grid->ctop[i];k<grid->Nk[i];k++) {
          ...
          //????
          if(prop->subgrid || !prop->wetdry)
            phys->stmp[i][k]+=(pow(phys->w[i][k],2)*subgrid->Acveffold[i][k]-pow(phys->w[i][k+1],2)*subgrid->Acveffold[i][k+1])/grid->dzz[i][k]/subgrid->Acceffold[i][k];            
          else
      ...
      for(i=0;i<grid->Nc;i++) {
        for(k=grid->ctop[i];k<grid->Nk[i];k++)
          if(!prop->subgrid && prop->wetdry)
            ...
            sum+=subgrid->Acceffold[i][k]*phys->stmp[i][k]*grid->dzz[i][k];
      }
  ...
  for(jptr=grid->edgedist[0];jptr<grid->edgedist[1];jptr++) {
    ...
    for(k=kmin;k<grid->Nke[j];k++) {
      ...
      if(!prop->subgrid)
      {
        ...
        phys->stmp[nc1][k]-=a[k]/subgrid->Acceffold[nc1][k]/grid->dzz[nc1][k]*grid->dzf[j][k];
        phys->stmp[nc2][k]+=a[k]/subgrid->Acceffold[nc2][k]/grid->dzz[nc2][k]*grid->dzf[j][k];        
      }
    ...
    for(k=grid->Nke[j];k<grid->Nk[nc1];k++) 
      if(!prop->subgrid)
        ...
        phys->stmp[nc1][k]+=0.25*prop->CdW*fabs(phys->w[nc1][k]+phys->w[nc1][k+1])*
         (phys->w[nc1][k]+phys->w[nc1][k+1])*grid->df[j]*grid->dzf[j][k]/grid->dzz[nc1][k]/subgrid->Acceffold[nc1][k];
    for(k=grid->Nke[j];k<grid->Nk[nc2];k++) 
      if(!prop->subgrid)
        ...
        phys->stmp[nc2][k]+=0.25*prop->CdW*fabs(phys->w[nc2][k]+phys->w[nc2][k+1])*
         (phys->w[nc2][k]+phys->w[nc2][k+1])*grid->df[j]*grid->dzf[j][k]/grid->dzz[nc2][k]/subgrid->Acceffold[nc2][k];      
  }
  ...
  for (jptr = grid->edgedist[4]; jptr < grid->edgedist[5]; jptr++){
    ...
    for (k=grid->ctop[nc]; k<grid->Nke[nc]; k++){
      if(!prop->subgrid)
        ...
        phys->stmp[nc][k]  += -2.0*prop->nu_H*
          (phys->boundary_w[boundary_index][k] - 0.5*(phys->w[nc][k] + phys->w[nc][k+1]))/grid->dg[j]*
          grid->df[j]/subgrid->Acceffold[nc][k];
    }
  }
...
static void ComputeQSource(REAL **src, gridT *grid, physT *phys, propT *prop, int myproc, int numprocs) 
{
  ...
  REAL *ap=phys->a, *am=phys->b, thetafactor=(1-prop->theta)/prop->theta,fac1,fac2,fac3,alpha,Ac;
  ...
  for(iptr=grid->celldist[0];iptr<grid->celldist[1];iptr++) {
    ...
    for(k=grid->ctop[i];k<grid->Nk[i];k++) {
      ...
      if(prop->subgrid)
      {
        alpha=subgrid->Acveff[i][k+1]/subgrid->Acveff[i][k];
        Ac=subgrid->Acveff[i][k];
      }
      // compute the vertical contributions to the source term
      src[i][k] = Ac*(phys->w[i][k]-alpha*phys->w[i][k+1]) 
        + fac2/fac1*Ac*(phys->wtmp2[i][k]-alpha*phys->wtmp2[i][k+1])+fac3/fac1*Ac*(phys->wtmp3[i][k]-alpha*phys->wtmp3[i][k+1]);
    }
    ...
    propT *prop, int myproc, int numprocs, MPI_Comm comm)
{
  ...
  REAL hmax,sum,sum0, dt=prop->dt, theta=prop->theta, h0, boundary_flag,fac1,fac2,fac3,tmp,tmp_x,tmp_y,tmp2;
  ...
  // subgrid sediment transport parameterization
  // add horiozontal pressure gradient explicit part
  if(prop->subgrid && grid->Nkmax==1)
    if(subgrid->erosionpara)
      for(iptr=grid->celldist[0];iptr<grid->celldist[1];iptr++) {
        i=grid->cellp[iptr];
        ...
        for(nf=0;nf<grid->nfaces[i];nf++)
        {
          ...
          if(nc2==-1)
            nc2=nc1;       
          tmp=prop->grav*
            (fac2*(phys->h[nc1]-phys->h[nc2])+fac3*(phys->hold2[nc1]-phys->hold2[nc2]))/grid->dg[ne]*
            grid->def[i*grid->maxfaces+nf]*grid->dzf[ne][0]*grid->df[ne]; 
          tmp2+=grid->def[i*grid->maxfaces+nf]*grid->dzf[ne][0]*grid->df[ne];
          tmp_x+=tmp*grid->n1[ne];
          tmp_y+=tmp*grid->n2[ne];     
        }        
        subgrid->tau_c_x[i]-=tmp_x/subgrid->Veff[i];
        subgrid->tau_c_y[i]-=tmp_y/subgrid->Veff[i];     
      } 
  ...
  for(jptr=grid->edgedist[0];jptr<grid->edgedist[1];jptr++) {
    ...
    //printf("j %d jptr %d top %d nke %d\n",j,jptr,grid->etop[j],grid->Nke[j]);
    //printf("nc1 %d dzz %e nc2 %d dzz %e\n",nc1,grid->dzz[nc1][grid->etop[j]],nc2,grid->dzz[nc2][grid->etop[j]]);
    if(!(grid->dzz[nc1][grid->etop[j]]==0 && grid->dzz[nc2][grid->etop[j]]==0)) {
        ...
        //fabs(phys->u[j][grid->etop[j]])*phys->u[j][grid->etop[j]];
          ...
          if(!prop->subgrid)
            phys->utmp[j][grid->Nke[j]-1]+=dt*(1-theta)*(
                ...
                (a[grid->Nke[j]-1] 
                 + 2.0*phys->CdB[j]*fabs(phys->ub_drag[j])/
            ...
            phys->utmp[j][grid->Nke[j]-1]+=dt*(1-theta)*(
                a[grid->Nke[j]-1]*phys->u[j][grid->Nke[j]-2] -
                (a[grid->Nke[j]-1] 
                 + phys->CdB[j]*fabs(phys->ub_drag[j])/
        ...
        if(phys->CdB[j] == -1){ // no slip on bottom
          ...
          if(!prop->subgrid)
            phys->utmp[j][grid->etop[j]]-=2.0*dt*(1-theta)*(phys->CdB[j])/
              (grid->dzz[nc1][grid->etop[j]]+grid->dzz[nc2][grid->etop[j]])*
              fabs(phys->ub_drag[j])*phys->u[j][grid->etop[j]];
          else
            phys->utmp[j][grid->etop[j]]-=dt*(1-theta)*(phys->CdB[j])/
              subgrid->dzboteff[j]*fabs(phys->ub_drag[j])*phys->u[j][grid->etop[j]];         
        }
        ...
        if(phys->CdT[j] == -1){ // no slip on top
          ...
          if(!prop->subgrid)
            phys->utmp[j][grid->etop[j]]-=2.0*dt*(1-theta)*(phys->CdT[j])/
              (grid->dzz[nc1][grid->etop[j]]+grid->dzz[nc2][grid->etop[j]])*
              fabs(phys->ut_drag[j])*phys->u[j][grid->etop[j]];
          else
            phys->utmp[j][grid->etop[j]]-=dt*(1-theta)*(phys->CdT[j])/
              subgrid->dzboteff[j]*fabs(phys->ut_drag[j])*phys->u[j][grid->etop[j]];          
        }
      }
      ...
      if(grid->Nke[j]-grid->etop[j]>1) { // for more than one vertical layer
        ...
        if(phys->CdT[j] == -1){ // no slip
          ...
          b[grid->etop[j]]=1.0+theta*dt*(b[grid->etop[j]]+
              2.0*phys->CdT[j]*fabs(phys->ut_drag[j])/
              (grid->dzz[nc1][grid->etop[j]]+
               grid->dzz[nc2][grid->etop[j]]));
        }
        ...
        if(phys->CdB[j] == -1){ // no slip
          ...
          if(!prop->subgrid)
            b[grid->etop[j]]+=2.0*theta*dt*fabs(phys->ub_drag[j])/
              (grid->dzz[nc1][grid->etop[j]]+grid->dzz[nc2][grid->etop[j]])*
              (phys->CdB[j]);
          else
            b[grid->etop[j]]+=theta*dt*fabs(phys->ub_drag[j])/
        ...
        if(phys->CdT[j] == -1){
          ...
          if(!prop->subgrid)
            b[grid->etop[j]]+=2.0*theta*dt*fabs(phys->ut_drag[j])/
              (grid->dzz[nc1][grid->etop[j]]+grid->dzz[nc2][grid->etop[j]])*
              (phys->CdT[j]);
          else
            b[grid->etop[j]]+=theta*dt*fabs(phys->ut_drag[j])/
      ...
      for(k=grid->etop[j];k<grid->Nke[j];k++) {
        ...
        //if(b[k]!=b[k] || b[k]==0) printf("n %d accnc1 %e accnc2 %e \n",prop->n,subgrid->Acceff[nc1][k],subgrid->Acceff[nc2][k] );        
  ...
  for(j=0;j<grid->Ne;j++) 
    for(k=grid->etop[j];k<grid->Nke[j];k++) 
      if(phys->utmp[j][k]!=phys->utmp[j][k]) {
        printf("n %d Error in function Predictor at j=%d k=%d (U***=nan) cd %e nc1 %d nc2 %d acc %e %e \n",prop->n,j,k,phys->CdB[j],grid->grad[2*j],grid->grad[2*j+1],\
                subgrid->Acceff[grid->grad[2*j]][k],subgrid->Acceff[grid->grad[2*j+1]][k]);
        printf("etop %d ctop %d %d dv %e %e h %e %e\n",grid->etop[j],grid->ctop[1890],grid->ctop[2],grid->dv[1890],grid->dv[2],phys->h[1890],phys->h[2] );
        exit(1);
      }
  ...
  for(iptr=grid->celldist[0];iptr<grid->celldist[1];iptr++) {
    ...
    // added for new wet dry method
    phys->htmp4[i]=phys->htmp[i];
  ...
  if(!prop->culvertmodel)
  {
     /*if(prop->subgrid){
      for(iptr=grid->celldist[0];iptr<grid->celldist[1];iptr++) {
        i = grid->cellp[iptr];
        if(grid->dv[i]+phys->h[i]<=2*DRYCELLHEIGHT)
        { 
          hmax=phys->h[i];
          for(nf=0;nf<grid->nfaces[i];nf++)
            if(grid->dzf[grid->face[i*grid->maxfaces+nf]][0]>0)
              if(phys->h[grid->neigh[i*grid->maxfaces+nf]]>hmax)
                hmax=phys->h[grid->neigh[i*grid->maxfaces+nf]];
          if(hmax!=phys->h[i])
          {
            //if(prop->n==850)
            printf("n %d i %d h %e hamx %e\n",prop->n,i,phys->h[i],hmax);
            phys->h[i]=(phys->h[i]+hmax)/2;
    ...
    while(1){
      for(iptr=grid->celldist[0];iptr<grid->celldist[1];iptr++) {
        i = grid->cellp[iptr];
        if(prop->subgrid)
          phys->htmp5[i]=-subgrid->Veff[i]+subgrid->Aceff[i]*phys->h[i];
        else
        {
          if(phys->h[i]>=-grid->dv[i])
            phys->htmp5[i]=-grid->Ac[i]*grid->dv[i];
          else
            phys->htmp5[i]=0;
        }
      }
      ...
      for(iptr=grid->celldist[0];iptr<grid->celldist[1];iptr++) {
        ...
        {
          if(phys->h[i]>=-grid->dv[i] && phys->htmp5[i]<0)
            phys->htmp5[i]+=grid->dv[i]*grid->Ac[i];
          else if(phys->h[i]>=-grid->dv[i] && phys->htmp5[i]==0)
            phys->htmp5[i]+=grid->Ac[i]*(grid->dv[i]+phys->h[i]);
          else if(phys->h[i]<-grid->dv[i] && phys->htmp5[i]<0)
            phys->htmp5[i]-=grid->Ac[i]*phys->h[i];
          else if(phys->h[i]<-grid->dv[i] && phys->htmp5[i]==0)
            phys->htmp5[i]=0;
          else
      ...
      ISendRecvCellData2D(phys->htmp5,grid,myproc,comm);
      sum=InnerProduct(phys->htmp5,phys->htmp5,grid,myproc,numprocs,comm);
    ...
    while(1)
    {
      ...
      while(1){
        ...
        for(iptr=grid->celldist[0];iptr<grid->celldist[1];iptr++) {
          i = grid->cellp[iptr];
          if(prop->subgrid)
            Culvertcondition[i]=-subgrid->Veff[i]+subgrid->Aceff[i]*phys->h[i];
          else
          {
            if(phys->h[i]>=-grid->dv[i])
              Culvertcondition[i]=-grid->Ac[i]*grid->dv[i];
            else
              Culvertcondition[i]=0;
          }
          // store formal iteration value
          phys->htmp5[i]=phys->h[i];
        }
        ...
        for(iptr=grid->celldist[0];iptr<grid->celldist[1];iptr++) {
          ...
          {
            if(phys->h[i]>=-grid->dv[i] && phys->htmp5[i]>=-grid->dv[i])
              Culvertcondition[i]+=grid->dv[i]*grid->Ac[i];
            else if(phys->h[i]>=-grid->dv[i] && phys->htmp5[i]<-grid->dv[i])
              Culvertcondition[i]+=grid->Ac[i]*(grid->dv[i]+phys->h[i]);
            else if(phys->h[i]<-grid->dv[i] && phys->htmp5[i]>=-grid->dv[i])
              Culvertcondition[i]-=grid->Ac[i]*phys->h[i];
            else if(phys->h[i]<-grid->dv[i] && phys->htmp5[i]<-grid->dv[i])
              Culvertcondition[i]=0;
            //else if(phys->h[i]>Culverttop[i] && phys->htmp5[i]>=-grid->dv[i])
            //  phys->htmp5[i]+=grid->Ac[i]*(Culverttop[i]+grid->dv[i]-phys->h[i]);
            //else if(phys->h[i]>Culverttop[i] && phys->htmp5[i]<-grid->dv[i])
            //  phys->htmp5[i]+=grid->Ac[i]*(Culverttop[i]+grid->dv[i]);
            else
              printf(" %dth cell has something wrong with the residual\n",i);
          }
        }
        ...
        ISendRecvCellData2D(Culvertcondition,grid,myproc,comm);

        // the total residual
        Culvertsum=InnerProduct(Culvertcondition,Culvertcondition,grid,myproc,numprocs,comm);
        ...
        if(sqrt(Culvertsum/sum0)<Culverteps)
          break;
      
        if(min>sqrt(Culvertsum/sum0))
          min=sqrt(Culvertsum/sum0);
      
        if(nf1>50)
        {
	        printf("culvert sum %e sum0 %e min %e\n",Culvertsum,sum0,min);
          //if(fabs(sqrt(Culvertsum/sum0)-min)<0.001)
          break;
        }
      ...
      for(iptr=grid->celldist[0];iptr<grid->celldist[1];iptr++) {
        i = grid->cellp[iptr];           
        Culvertcondition2[i]=CulvertQcoef[i]*(Culverttop[i]-phys->h[i]);
        Culvertpressure2[i]=phys->h[i];
      } 
      ...
      // calculate norm error
      ISendRecvCellData2D(Culvertcondition2,grid,myproc,comm);

      // the total residual
      Culvertsum=InnerProduct(Culvertcondition2,Culvertcondition2,grid,myproc,numprocs,comm);
      ...
      // check culvert condition
      if(sqrt(Culvertsum/sum)<Culverteps)
        break; 

      if(min2>sqrt(Culvertsum/sum))
        min2=sqrt(Culvertsum/sum);

      if(nf>50) { 
        printf("n %d nf %d something maybe wrong for convergence at time step min %e sum %e sum0 %e r %e\n",prop->n,nf,min,Culvertsum,sum,sqrt(Culvertsum/sum));
        if(fabs(sqrt(Culvertsum/sum)-min2)<0.001)
          break;
      }
    }
  }
  ...
  for(i=0;i<grid->Nc;i++){
    ...
    //printf("i %d xv %e yv %e s %e h+d %e \n",i,grid->xv[i],grid->yv[i],phys->s[i][0],grid->dv[i]+phys->h[i]);  
    if(phys->h[i]<=(-grid->dv[i]+DRYCELLHEIGHT)) {
    ...
    /*if(prop->subgrid)
    {
      if(subgrid->Veff[i]<=(DRYCELLHEIGHT*grid->Ac[i]))
      {
        ...
        phys->active[i]=0;
        phys->s[i][grid->Nk[i]-1]=0;
        if(prop->computeSediments && prop->n>=2)
          for(k=0;k<sediments->Nsize;k++)
            sediments->SediC[k][i][grid->Nk[i]-1]=0;
      } else {
        phys->hcorr[i]=0;
  ...
  if(prop->subgrid){
    ...
    // subgrid sediment transport parameterization
  // add back pressure gradient implicit part
      ...
      for(iptr=grid->celldist[0];iptr<grid->celldist[1];iptr++) {
        ...
        for(nf=0;nf<grid->nfaces[i];nf++)
        {
          ...
          if(nc2==-1)
            nc2=nc1;       
          tmp=prop->grav*
            fac1*(phys->h[nc1]-phys->h[nc2])/grid->dg[ne]*
            grid->def[i*grid->maxfaces+nf]*grid->dzf[ne][0]*grid->df[ne];
          tmp_x+=tmp*grid->n1[ne];
          tmp_y+=tmp*grid->n2[ne];
          //if(i==6) 
            //printf("4444444 ne %d dhdx %e tau_c_x %e tmp_x %e u %e uc %e dfAf %e\n", ne,prop->grav*(phys->h[nc1]-phys->h[nc2])/grid->dg[ne]*grid->n1[ne],
              //subgrid->tau_c_x[i],tmp_x,phys->u[ne][0]*grid->n1[ne],phys->uc[i][0],grid->def[i*grid->maxfaces+nf]*grid->dzf[ne][0]*grid->df[ne]);  
        }
        /*if(i==6) 
          printf("555555tau_c_x %e tmp_x %e tmp2 %e uc %e v %e\n", subgrid->tau_c_x[i],
            tmp_x/subgrid->Veff[i],tmp2,phys->uc[i][0],subgrid->Veff[i]);  */
        subgrid->tau_c_x[i]-=tmp_x/subgrid->Veff[i];
        subgrid->tau_c_y[i]-=tmp_y/subgrid->Veff[i];
      }  
  ...
  /*if(prop->subgrid)
    for(i=0;i<grid->Ne;i++)
    {
      ...
      if(subgrid->Veff[nc1]<=(DRYCELLHEIGHT*grid->Ac[nc1]) && subgrid->Veff[nc2]<=(DRYCELLHEIGHT*grid->Ac[nc2]))
        phys->u[i][grid->Nke[i]-1]=0;
    }
  */
  ...
  /*if(prop->n==2){ 
     sum=0;
     for(nf=0;nf<grid->nfaces[0];nf++)
     {
       ne=grid->face[0*grid->maxfaces+nf];
       for(k=grid->etop[ne];k<grid->Nke[ne];k++){
         sum+=(0.55*phys->u[ne][k]+0.45*phys->utmp2[ne][k])*
            grid->df[ne]*normal*grid->dzf[ne][k];
       printf("ne %d k %d dzf %e u %e\n", ne,k,grid->dzf[ne][k],phys->u[ne][k]);
       }
     }
     printf("sum %e veff %e veffold %e d %e %e\n",sum,subgrid->Veff[0]-subgrid->Veffold[0],subgrid->Veffold[0],grid->dv[0]+phys->h[0],phys->hold2[0]+grid->dv[0]);
  }*/

  //if(prop->n==5133)
    //for(i=0;i<grid->Nc;i++)
      //printf("i %d %e ctop %d dzz %e dv %e\n",i,phys->s[i][0],grid->ctop[i],grid->dzz[i][0],grid->dv[i]);
...
//static void Continuity(REAL **w, gridT *grid, physT *phys, propT *prop)
{
  ...
  REAL ap, am, dzfnew, theta=prop->theta,fac1,fac2,fac3,Ac,alpha,sum;
    ...
    for(k=grid->ctop[n];k<grid->Nk[n];k++) {
      ...
      for(nf=0;nf<grid->nfaces[n];nf++) {
        ...
        if(subgridmodel && grid->ctop[n]==grid->Nk[n]-1)
          if(grid->def[n*grid->maxfaces+nf]*grid->dzf[ne][k]*grid->df[ne]/subgrid->Acceff[n][k]/grid->dzz[n][k]<=1)
...
{ 
  ...
  *prop = (propT *)SunMalloc(sizeof(propT),"ReadProperties");
  ...
  (*prop)->udrag = MPI_GetValue(DATAFILE,"udrag","ReadProperties",myproc);
  ...
  (*prop)->hiter = MPI_GetValue(DATAFILE,"hiter","ReadProperties",myproc);
  ...
  if((*prop)->subgrid && !(*prop)->hiter)
  {
    (*prop)->hiter=1;
    printf("hiter module is started automatically because subgrid is activated. \n");
  }
...
//          +Min(grid->dv[nc1],grid->dv[nc2]));
      //if(fabs(phys->u[j][k])>0.001)
        grid->dzf[j][k]=Max(0,UpWind(phys->u[j][k],phys->h[nc1],phys->h[nc2])+Min(grid->dv[nc1],grid->dv[nc2]));
      //else
        //grid->dzf[j][k]=Max(phys->h[nc1],phys->h[nc2])+Min(grid->dv[nc1],grid->dv[nc2]);
...
/*
 * Function: ComputeUDrag
 * Usage: CompuateUDrag(u_drag, uc, vc, h, grid, myproc,udrag)
 * -------------------------------------------
 * compute udrag based on uc and vc at the former time step
 *
 */
static void ComputeUDrag(physT *phys, propT *prop, gridT *grid, int myproc, MPI_Comm comm)
{
  int jptr, j, nc1, nc2, k;
  REAL def1,def2,dgf,u_f,v_f;

  if(prop->udrag){
    // first calculate u_tilt=u^n-dt/2*(dh/dh)^n
    for(jptr=grid->edgedist[0];jptr<grid->edgedist[1];jptr++) {
      j = grid->edgep[jptr];
      nc1 = grid->grad[2*j];
      nc2 = grid->grad[2*j+1];
      ...
      for(k=grid->etop[j];k<grid->Nke[j];k++)
        phys->u_tilt[j][k]=phys->u[j][k]-0.5*prop->dt*prop->grav*(phys->h[nc1]-phys->h[nc2])/grid->dg[j];
    }
    
    // calculate uc_tilt and vc_tilt
    ComputeUCPerot(phys->u_tilt,phys->uc_tilt,phys->vc_tilt,phys->h,prop->kinterp,prop->subgrid,grid);
    // now send interprocessor data
    ISendRecvCellData3D(phys->uc_tilt,grid,myproc,comm);
    ISendRecvCellData3D(phys->vc_tilt,grid,myproc,comm);
    ...
    for(jptr=grid->edgedist[0];jptr<grid->edgedist[1];jptr++) {
      ...
      u_f=def1/dgf*phys->uc_tilt[nc1][grid->Nke[j]-1]+def2/dgf*phys->uc_tilt[nc2][grid->Nke[j]-1];
      v_f=def1/dgf*phys->vc_tilt[nc1][grid->Nke[j]-1]+def2/dgf*phys->vc_tilt[nc2][grid->Nke[j]-1];
      phys->ub_drag[j]=sqrt(pow(u_f,2)+pow(v_f,2));

      u_f=def1/dgf*phys->uc_tilt[nc1][grid->etop[j]]+def2/dgf*phys->uc_tilt[nc2][grid->etop[j]];
      v_f=def1/dgf*phys->vc_tilt[nc1][grid->etop[j]]+def2/dgf*phys->vc_tilt[nc2][grid->etop[j]];
      phys->ut_drag[j]=sqrt(pow(u_f,2)+pow(v_f,2));

    }
  } else {
    for(jptr=grid->edgedist[0];jptr<grid->edgedist[1];jptr++) {
      j = grid->edgep[jptr];
      phys->ub_drag[j]=phys->u[j][grid->Nke[j]-1];
      phys->ut_drag[j]=phys->u[j][grid->etop[j]];
    }
  }
}
