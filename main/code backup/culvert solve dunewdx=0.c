...
void AllocateCulvert(gridT *grid,int myproc)
{
  ...
  Culvertpressure2 = (REAL *)SunMalloc(grid->Nc*sizeof(REAL), "AllocateCulvert");
  Culverttoparea = (REAL *)SunMalloc(grid->Nc*sizeof(REAL), "AllocateCulvert");
  CulvertQcoef = (REAL *)SunMalloc(grid->Nc*sizeof(REAL), "AllocateCulvert");
  Culvertcondition = (REAL *)SunMalloc(grid->Nc*sizeof(REAL), "AllocateCulvert");
  Culvertcondition2 = (REAL *)SunMalloc(grid->Nc*sizeof(REAL), "AllocateCulvert");
...
void FreeCulvert(int myproc)
{
  ...
  free(Culvertcondition2);
  free(Culvertpressure);
  free(Culvertpressure2);
  free(CulvertQcoef);
  free(Culvertsource1);
  free(Culvertsource2);
  free(Culvertsource3);
...
{ 
  ...
  for(i=0;i<grid->Nc;i++){
    ...
    Culvertcondition2[i]=0; //assume free surface first
    Culvertsource1[i]=0;
    Culvertsource2[i]=0;
    Culvertsource3[i]=0;
    Culvertsource4[i]=0;
...
void CulvertHCoefficients(REAL *coef, REAL *fcoef, gridT *grid, physT *phys, propT *prop, int myproc)
{
  ...
  for(iptr=grid->celldist[0];iptr<grid->celldist[1];iptr++) {
    ...
    if(!prop->subgrid){   
      ...
      if(phys->h[i]<-grid->dv[i] && Culvertpressure[i]<Culverttop[i])
        coef[i] = 0-CulvertQcoef[i];
      else
        coef[i] = grid->Ac[i]-CulvertQcoef[i];

      if(Culvertpressure[i]>Culverttop[i] && phys->h[i]>Culverttop[i])
        coef[i]=0;

      //if(coef[i]<0)
        //printf("i %d qcoef %e\n",i,CulvertQcoef[i]);
    } else {
      coef[i]=subgrid->Aceff[i]-CulvertQcoef[i];
...
{ 
  ...
  if(no==1){
    for(i=0;i<Nc;i++){
      ...
      Culvertpressure2[i]=h[i]; //store new pressure field (for culvert part maybe not freesurface)
...
{ 
  ...
  for(iptr=grid->celldist[0];iptr<grid->celldist[1];iptr++) {
    ...
    if(!prop->subgrid){
      ...
      if(Culvertpressure2[i]>Culverttop[i])
        phys->htmp[i]-=Culverttoparea[i]*Culverttop[i];

      if(Culvertpressure[i]>Culverttop[i] && phys->h[i]>Culverttop[i])
      {
        phys->htmp[i]=Culvertsource1[i]-Culverttoparea[i]*Culverttop[i]; 
        sum = 0;
      ...
      if(Culvertpressure[i]>Culverttop[i] && phys->h[i]>Culverttop[i])
      {
        phys->htmp[i]=Culvertsource1[i]-subgrid->Veff[i]; 
        sum = 0;
        // add back explicit part
        for(nf=0;nf<grid->nfaces[i];nf++) {
          ne = grid->face[i*grid->maxfaces+nf];
  ...
  if(no==-1){
    ...
    for(iptr=grid->celldist[0];iptr<grid->celldist[1];iptr++) {
      ...
      if(phys->h[i]>=Culverttop[i] && !prop->subgrid){
        phys->htmp[i]=phys->htmp[i]-grid->Ac[i]*phys->h[i]+Culverttoparea[i]*Culverttop[i];
      }    
 ...
 * usage: Update Qcoef for outer loop based on Culvertpressure2
 -----------------------------------------------------------------------------------
 * 
 */
void UpdateCulvertQcoef(gridT *grid, propT *prop,int no, int myproc)
{
   ...
   if(no==0)
     for(nc=0;nc<grid->Nc;nc++)
       if(Culvertpressure2[nc]>Culverttop[nc])
         CulvertQcoef[nc]=Culverttoparea[nc];
       else
         CulvertQcoef[nc]=0;
   else 
     CulvertQcoef[nc]=0;   
