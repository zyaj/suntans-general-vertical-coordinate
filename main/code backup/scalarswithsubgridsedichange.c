    ...
    MPI_Comm comm, int myproc, int checkflag, int TVDscheme) 
{
  ...
  REAL smin, smax, div_local, div_da, alpha;
    ...
    if(src1)
      if(!prop->computeSediments)
        ...
        if(!sediments->scalaroption)
          ...
          {  
            if(prop->subgrid)
              alpha=subgrid->Aceffold[i]/Ac;
            else 
              alpha=1.0;
            d[k-ktop]-=alpha*src1[i][k]*(1-theta)*dt*grid->dzz[i][k]*phys->stmp[i][k];
          }
      }
          ...
          if(!sediments->scalaroption)
            ...
            for(k=grid->ctopold[i];k<=grid->ctop[i];k++)
            {
              if(prop->subgrid)
                ...
                alpha=1.0;
              d[0]-=alpha*src1[i][k]*(1-theta)*dt*grid->dzz[i][k]*phys->stmp[i][k];
            }
       }
       ...
       if(!prop->computeSediments){
         ...
         if(!sediments->scalaroption)
         {
          ...
          if(prop->subgrid)
            ...
            alpha=1.0;
          d[0]-=alpha*src1[i][ktop]*(1-theta)*dt*grid->dzz[i][ktop]*phys->stmp[i][k]; 
         }
       }
    ...
    if(src2)
      if(!prop->computeSediments)
        ...
        if(!sediments->scalaroption)
          ...
          { 
            if(prop->subgrid)
              alpha=((1-theta)*subgrid->Aceffold[i]+theta*subgrid->Aceff[i])/Ac;
            else 
              alpha=1.0; 
      ...
      if(!flag) {
        ...
        for(k=0;k<grid->Nk[i];k++) {
          ...
          if(!prop->subgrid)
          {
            ...
            div_local+=(theta*(wnew[i][k]*subgrid->Acveff[i][k]-subgrid->Acveff[i][k+1]*wnew[i][k+1])+
                (1-theta)*(subgrid->Acveffold[i][k]*phys->wtmp2[i][k]
