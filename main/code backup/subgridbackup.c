...
void InterpolateSubgridDepth(gridT *grid, physT *phys, int myproc)
{
  ...
  if(subgrid->dpint==1)
  {
    ...
    MPI_GetFile(str,DATAFILE,"subpointinFile","InterpolateSubgridDepth",myproc);
    ifile = MPI_FOpen(str,"r","InterpolateSubgridDepth",myproc);
    for(nc=0;nc<grid->Nc;nc++){
      ncount=(grid->nfaces[nc]-2)*(subgrid->segN+1)*(subgrid->segN+2)/2;
      base=nc*(grid->maxfaces-2)*(subgrid->segN+1)*(subgrid->segN+2)/2;
    ...
    MPI_GetFile(str,DATAFILE,"subpointeinFile","InterpolateSubgridDepth",myproc);
    ifile = MPI_FOpen(str,"r","InterpolateSubgridDepth",myproc);
    ncount=grid->Ne*(subgrid->segN+1);
    for(n=0;n<ncount;n++)
    {
  ...
  for(i=0;i<grid->Nc;i++)
  {
    ...
    for(j=0;j<=subgrid->disN;j++)
    {
      ...
      if(j!=subgrid->disN && subgrid->Acprof[base+j]>subgrid->Acprof[base+j+1])
        printf("i %d cell j %dth Acprof is %f is bigger than j+1 %d th\n",i,j,subgrid->Acprof[base+j],(j+1));
...
void UpdateSubgridVerticalAceff(gridT *grid, physT *phys, propT *prop, int option, int myproc)
{
  ...
  if(option==0){
    for(i=0;i<grid->Nc;i++)
    {
      ...
      /*if(i==36)
      {
        printf("dasdsada ac %e dznew%e %e %e \n",subgrid->Acceff[i][0],grid->dzz[i][0],DRYCELLHEIGHT,(grid->dzz[i][0]-DRYCELLHEIGHT));
        exit(1);
      }*/
...
void UpdateSubgridFluxHeight(gridT *grid, physT *phys, propT *prop, int myproc)
{
  ...
  for(ne=0;ne<grid->Ne;ne++)
  {
   ...
   // if(prop->culvertmodel==1)
    ...
    k=grid->Nke[ne]-1;
    if(grid->etop[ne]==k)
      if(subgrid->Veff[nc]<=grid->Ac[nc]*DRYCELLHEIGHT)
	grid->dzf[ne][k]=0;
    ...
    if(subgrid->dzboteff[ne]<DRYCELLHEIGHT)
...
REAL UpdateFluxHeight(int ne, REAL h)
{
  ...
  else if(h>=subgrid->hprofe[base+subgrid->disN])
  {
    ...
    //if(ne==2618)
      //printf("\nh %e fh %e hf1 %e fx1 %e hf2 %e fx2 %e\n",h,fh,subgrid->hprofe[base+i],subgrid->fluxhprof[base+i],subgrid->hprofe[base+i-1],subgrid->fluxhprof[base+i-1]);
  }
  return fh;
}
...
void UpdateSubgridHeff(gridT *grid, physT *phys, propT *prop, int myproc)
{
  ...
  for(i=0;i<grid->Nc;i++)
  {
    ...
    if(subgrid->Aceff[i]/grid->Ac[i]<0.00001)
      subgrid->Heff[i]=DRYCELLHEIGHT;
  }
}

/*
 * Function: CalculateActiveErosion
...
{
  REAL h,hcrit,heff,Hcrit,d1,d2,d3,utmp,ratio1,ratio2,ratio, ac,em,alpha,erosionmax,nettmp,beta;
  int i,j,k; 
