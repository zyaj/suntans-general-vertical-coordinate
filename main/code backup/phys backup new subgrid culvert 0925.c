...
static void WPredictor(gridT *grid, physT *phys, propT *prop,
    int myproc, int numprocs, MPI_Comm comm);
inline void ComputeUC(REAL **ui, REAL **vi, physT *phys, gridT *grid, int myproc, interpolation interp,int kinterp);
static void ComputeUCPerot(REAL **u, REAL **uc, REAL **vc, REAL *h, int kinterp, gridT *grid);
...
void InitializePhysicalVariables(gridT *grid, physT *phys, propT *prop, int myproc, MPI_Comm comm)
{
  ...
  */
  ...
  ComputeUC(phys->uc, phys->vc, phys, grid, myproc, prop->interp,prop->kinterp);
  ComputeUC(phys->uold, phys->vold, phys, grid, myproc, prop->interp,prop->kinterp);
    ...
    if(prop->z0B==0) 
      ...
      for(j=0;j<grid->Ne;j++) 
      {
       if(!prop->subgrid)
         ...
         //dz=grid->dzf[j][grid->Nke[j]-1];
       else
         phys->CdB[j]=pow(log(0.5*subgrid->dzboteff[j]/prop->z0B)/KAPPA_VK,-2);
         //dz=subgrid->dzboteff[j];
         //printf("ne %d cdb %e dz %e\n",j,phys->CdB[j],dz);
...
void Solve(gridT *grid, physT *phys, propT *prop, int myproc, int numprocs, MPI_Comm comm)
{
  ...
  for(n=prop->nstart+1;n<=prop->nsteps+prop->nstart;n++) {
    ...
    if (prop->outputNetcdf==0){
      ...
      if(prop->marshmodel==1 && prop->n==prop->nstart+1)
...
static void UPredictor(gridT *grid, physT *phys, 
    propT *prop, int myproc, int numprocs, MPI_Comm comm)
{
  int i, iptr, j, jptr, ne, nf, nf1, normal, nc1, nc2, k, n0, n1,iv,jv, botinterp;
  REAL sum,sum0, dt=prop->dt, theta=prop->theta, h0, boundary_flag;
...
//            fabs(phys->u[j][grid->etop[j]])*phys->u[j][grid->etop[j]];
	...
	phys->utmp[j][grid->Nke[j]-1]-=prop->dt*(1-prop->thetaM)*(a0[grid->Nke[j]-1]*phys->u[j][grid->Nke[j]-2]
      ...
      if(prop->marshmodel==1)
        for(jv=marshtop[j];jv<grid->Nke[j];jv++)
  ...
  if(prop->culvertmodel==0)
  {
    ...
    while(1){
      for(iptr=grid->celldist[0];iptr<grid->celldist[1];iptr++) {
        ...
        if(i==18175 && prop->n==6139)
          printf("\n n %d nf %d 111 i %d htmp %12.11e veff %21.20e aceff %21.20e h %21.20e dv %12.11e \n",prop->n,nf,i,phys->htmp5[i],subgrid->Veff[i],subgrid->Aceff[i],phys->h[i],grid->dv[i]);
      }
      ...
      for(iptr=grid->celldist[0];iptr<grid->celldist[1];iptr++) {
        i = grid->cellp[iptr];
        //if((phys->hold2[i]-phys->h[i])*(phys->dhdt[i]-subgrid->Veff[i])<0)
          //printf("n %d nf %d i %d something wrong with the calculation of volume hold %21.20e h %21.20e vold %21.20e v %21.20e dv %21.20e\n",prop->n,nf,i,phys->hold2[i],phys->h[i],phys->dhdt[i],subgrid->Veff[i],grid->dv[i]);
      ...
      for(iptr=grid->celldist[0];iptr<grid->celldist[1];iptr++) {
        ...
        if(fabs(phys->htmp5[i])<1e-5)
          phys->htmp5[i]=0; 

        if(prop->n==6139 && i==18175)// phys->htmp5[i]>1)
          printf("\n n %d nf %d  222 i %d htmp %12.11e veff %21.20e aceff %21.20e h %21.20e\n",prop->n, nf, i,phys->htmp5[i],subgrid->Veff[i],subgrid->Aceff[i],phys->h[i]);
      }     
      ...
      if(prop->subgrid)
        UpdateSubgridAceff(grid, phys, prop, myproc);
      //if(prop->n==949 && nf==19966)
      //  printf("aceff %e\n",subgrid->Aceff[61]);
      if(sqrt(sum/sum0)<0.001)
        break;
    ...
    // outer loop
    UpdateCulvertQcoef(grid,prop, 1, myproc);
    ...
    while(1)
    {
      ...
      {
        i = grid->cellp[iptr];
        if(Culverttop[i]!=INFTY)
          phys->h[i]=Culverttop[i]+0.1; 
      }*/
      ...
      while(1){
        ...
        for(iptr=grid->celldist[0];iptr<grid->celldist[1];iptr++) {
          ...
          else
          {
            ...
            //  Culvertcondition[i]=-grid->Ac[i]*(Culverttop[i]+grid->dv[i]-phys->h[i]);
          }
        ...
        CGSolve(grid,phys,prop,myproc,numprocs,comm);
        // calculate error vector for convergence
        //CheckCulvertCondition(phys->h,grid,myproc);
        ...
        for(iptr=grid->celldist[0];iptr<grid->celldist[1];iptr++) {
          ...
          else
          {
            ...
            //if(Culvertcondition[i]>0.1)
              //printf("i %d condition %e h %e htmp %e dv %e top %e \n",i,Culvertcondition[i],phys->h[i],phys->htmp5[i],grid->dv[i],Culverttop[i]);
          }
        }

        //for(iptr=0;iptr<grid->Nc;iptr++)
        //  printf("iprt %d condition %e \n",iptr,Culvertcondition[iptr]); 
        ...
        //printf("nf1 %d min %e n %d culvertsum %e %e \n",nf1,min, prop->n, Culvertsum,fabs(sqrt(Culvertsum)-min));
        //if(nf1>50)
          //break;
      }
      ...
      for(iptr=grid->celldist[0];iptr<grid->celldist[1];iptr++) {
        ...
        //if(CulvertQcoef[i]!=0)
          //printf("nf %d 1  i %d Qcoef %e xv %e Condition %e\n",nf,i,CulvertQcoef[i],grid->xv[i],Culvertcondition2[i]);
      } 
      ...
      for(iptr=grid->celldist[0];iptr<grid->celldist[1];iptr++) {
        ...
        Culvertcondition2[i]+=CulvertQcoef[i]*(-Culverttop[i]+phys->h[i]);
        //if(CulvertQcoef[i]!=0)
          //printf("nf %d 2  i %d Qcoef %e xv %e condition %e \n",nf, i,CulvertQcoef[i],grid->xv[i],Culvertcondition2[i]);
      } 
      ...
      if(nf==1)
        sum0=Culvertsum;
      //printf("n %d culversum %e  sum0 %e \n ",prop->n, Culvertsum,sum0);
      ...
      if(min>sqrt(Culvertsum))
        min=sqrt(Culvertsum);
      if(nf>50 && fabs(min-sqrt(Culvertsum))<0.001)
        break;
    }
  }
  ...
  for(i=0;i<grid->Nc;i++){
    ...
    //else
     // min=subgrid->hmin[i];
    if(phys->h[i]<=min+DRYCELLHEIGHT) {
      //phys->hcorr[i]=-grid->dv[i]+DRYCELLHEIGHT-phys->h[i];
      phys->h[i]=min+DRYCELLHEIGHT;
      phys->active[i]=0;
  ...
  for(jptr=grid->edgedist[0];jptr<grid->edgedist[1];jptr++) {
    ...
    // CHANGE
    if(grid->etop[j]==grid->Nke[j]-1 && grid->dzz[nc1][grid->etop[j]]<=DRYCELLHEIGHT &&
        grid->dzz[nc2][grid->etop[j]]<=DRYCELLHEIGHT) 
...
static void HCoefficients(REAL *coef, REAL *fcoef, gridT *grid, physT *phys, propT *prop) {
  ...
  for(iptr=grid->celldist[0];iptr<grid->celldist[1];iptr++) {
    ...
    for(nf=0;nf<grid->nfaces[i];nf++) 
      if(grid->neigh[i*grid->maxfaces+nf]!=-1) {
        ...
        fcoef[i*grid->maxfaces+nf]=tmp*phys->D[ne]*grid->df[ne]/grid->dg[ne];
         //if(i==9749) // ne==20993 ){
          //printf("\n i %d h %e dv %e ne %d dzf %e \n phys->D %e df %e dg %e fcoef %e u %e dzz %e dv+h %e\n",i, phys->h[i], grid->dv[i], ne,grid->dzf[ne][0],phys->D[ne],grid->df[ne],grid->dg[ne],fcoef[i*grid->maxfaces+nf],phys->u[ne][0],grid->dzz[i][0],(grid->dv[i]+phys->h[i]));
    ...
    if(prop->n==6139 && i==18175){
      printf("%d htmp %e htmp4 %e coef %e h %e dv %e ac %e D %e\n",i,phys->htmp[i],phys->htmp4[i],coef[i],phys->h[i],grid->dv[i],grid->Ac[i],phys->D[ne],grid->dzf[ne][0]);
      //printf("veff %e aceff %e  %e",subgrid->Veff[i],subgrid->Aceff[i],(subgrid->Veff[i]-subgrid->Aceff[i]*phys->h[i]));
      for(nf=0;nf<grid->nfaces[i];nf++)
        printf("fcoef %e ",fcoef[i*grid->maxfaces+nf]);
      printf("\n");
    }
...
{ 
  ...
  *prop = (propT *)SunMalloc(sizeof(propT),"ReadProperties");
  ...
  // now subgrid can only work for 2d case
  /*if(grid->Nkmax!=1 && (*prop)->subgrid==1)
  {
    printf("subgrid module is closed automatically because the problem is not 2D.\n");
    (*prop)->subgrid=0;
  }*/
...
//          +Min(grid->dv[nc1],grid->dv[nc2]));
      grid->dzf[j][k]=Max(0,UpWind(phys->u[j][k],phys->h[nc1],phys->h[nc2])+Min(grid->dv[nc1],grid->dv[nc2]));
    //if(j==287)
      //printf("ne %d dzf-dry %e upwind %e min %e hnc1 %e hnc2 %e\n",j,(grid->dzf[j][k]-DRYCELLHEIGHT),UpWind(phys->u[j][k],phys->h[nc1],phys->h[nc2]),Min(grid->dv[nc1],grid->dv[nc2]),phys->h[nc1],phys->h[nc2]);
      // added part
    ...
    /*
       ...
       */
    //if(j==287)
      //printf("aaane %d dzf-dry %e upwind %e min %e hnc1 %e hnc2 %e\n",j,(grid->dzf[j][k]-DRYCELLHEIGHT),UpWind(phys->u[j][k],phys->h[nc1],phys->h[nc2]),Min(grid->dv[nc1],grid->dv[nc2]),phys->h[nc1],phys->h[nc2]);
    for(k=grid->etop[j];k<grid->Nke[j];k++) 
    {
      //if(j==287)
        //printf("1k %d dsadsada %e \n",k,grid->dzf[j][k]-DRYCELLHEIGHT);
      if(grid->dzf[j][k]<=DRYCELLHEIGHT)
      {
        grid->dzf[j][k]=0;
      }
      //if(j==287)
        //printf("2k %d dsadsada %e \n",k,grid->dzf[j][k]-DRYCELLHEIGHT);
    }
    //if(j==287) printf("%e %e \n",grid->dzf[j][k],grid->dzf[j][k]-DRYCELLHEIGHT);
    //if(j==287)
      //printf("fasadane %d dzf-dry %e upwind %e min %e hnc1 %e hnc2 %e\n",j,(grid->dzf[j][k]-DRYCELLHEIGHT),UpWind(phys->u[j][k],phys->h[nc1],phys->h[nc2]),Min(grid->dv[nc1],grid->dv[nc2]),phys->h[nc1],phys->h[nc2]);
    for(k=grid->etop[j];k<grid->Nke[j];k++)
    {
...
inline void ComputeUC(REAL **ui, REAL **vi, physT *phys, gridT *grid, int myproc, interpolation interp,int kinterp) {
  ...
  switch(interp) {
    case QUAD:
      ...
      break;
    case PEROT:
      ComputeUCPerot(phys->u,ui,vi,phys->h,kinterp,grid);
      break;
  }
