...
void ReadDepth(gridT *grid, int myproc) {
  int n,scaledepth;
  REAL depthelev,scaledepthfactor;
  char str[BUFFERLENGTH];
  FILE *fid;
  sprintf(str,"%s-voro",INPUTDEPTHFILE);
  scaledepth=(int)MPI_GetValue(DATAFILE,"scaledepth","ReadDepth",myproc);
  scaledepthfactor=MPI_GetValue(DATAFILE,"scaledepthfactor","ReadDepth",myproc);
  depthelev=MPI_GetValue(DATAFILE,"depthelev","ReadDepth",myproc);
